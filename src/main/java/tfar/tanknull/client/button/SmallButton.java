...
public class SmallButton extends Button {
  ...
  public boolean shouldDrawText(){
    return false;//!getMessage().isEmpty();
  }
  ...
  public void render(MatrixStack stack,int mouseX, int mouseY, float partialTicks) {
    if (visible) {
      TankNullClient.mc.getTextureManager().bindTexture(WIDGETS_LOCATION);
  ...
  public void drawText(MatrixStack stack,int halfwidth2){
    ...
    this.drawCenteredString(stack,TankNullClient.mc.fontRenderer, getMessage(), x + halfwidth2, y + (this.height - 8) / 2, textColor);
  }
}